...
int credential_loader_load_from_json(const char *json_file) {
  ...
  for (size_t i = 0; i < count; i++) {
    ...
    struct json_object *username_obj =
        json_object_object_get(cred_obj, "username");
    struct json_object *password_obj =
        json_object_object_get(cred_obj, "password");
    ...
    struct json_object *confidence_obj =
        json_object_object_get(cred_obj, "confidence");

    weighted_credential_t *cred = &g_pool.credentials[i];

    strncpy(cred->username, json_object_get_string(username_obj),
            sizeof(cred->username) - 1);
    strncpy(cred->password, json_object_get_string(password_obj),
            sizeof(cred->password) - 1);

    cred->weight = weight_obj ? json_object_get_double(weight_obj) : 1.0;
    cred->confidence =
        confidence_obj ? json_object_get_double(confidence_obj) : 0.5;

    if (source_obj) {
      strncpy(cred->source, json_object_get_string(source_obj),
              sizeof(cred->source) - 1);
    } else {
  ...
  log_info("Loaded %zu credentials from %s (total_weight=%.2f)", count,
           json_file, g_pool.total_weight);
...
credential_t credential_loader_get_weighted(void) {
  ...
  for (size_t i = 0; i < g_pool.count; i++) {
    ...
    if (random_val <= cumulative_weight) {
      strncpy(result.username, g_pool.credentials[i].username,
              sizeof(result.username) - 1);
      strncpy(result.password, g_pool.credentials[i].password,
              sizeof(result.password) - 1);
      break;
    }
  }
...
credential_t credential_loader_get_by_index(size_t index) {
  ...
  if (index < g_pool.count) {
    strncpy(result.username, g_pool.credentials[index].username,
            sizeof(result.username) - 1);
    strncpy(result.password, g_pool.credentials[index].password,
            sizeof(result.password) - 1);
  }
...
void credential_loader_update_result(const char *username, const char *password,
  ...
  for (size_t i = 0; i < g_pool.count; i++) {
    ...
    if (strcmp(cred->username, username) == 0 &&
        strcmp(cred->password, password) == 0) {
      cred->attempts++;
      if (success) {
        cred->successes++;
      }
      ...
      log_debug("Updated credential %s:%s - attempts=%u, successes=%u, "
                "rate=%.2f%%, weight=%.2f",
...
void credential_loader_print_stats(void) {
  ...
  for (size_t i = 0; i < g_pool.count && i < 10; i++) {
    weighted_credential_t *cred = &g_pool.credentials[i];
    if (cred->attempts > 0) {
      log_info("  %s:%s - %u/%u (%.1f%%) weight=%.2f source=%s", cred->username,
               cred->password, cred->successes, cred->attempts,
               cred->success_rate * 100, cred->weight, cred->source);
    }
  }
