...
/* Use linux/ headers only — they supersede the netinet/ versions for raw
 * sockets. Including both causes struct redefinition errors on Linux. */
#include <fcntl.h>
#include <linux/ip.h>
#include <linux/tcp.h>
...
int syn_scanner_init(syn_scanner_t *scanner, uint32_t local_ip) {
  ...
  if (scanner->raw_sock < 0) {
    log_error("Failed to create raw socket: %s (requires CAP_NET_RAW)",
              strerror(errno));
    return -1;
  }
  ...
  int one = 1;
  if (setsockopt(scanner->raw_sock, IPPROTO_IP, IP_HDRINCL, &one, sizeof(one)) <
      0) {
 ...
 */
static int send_syn_packet(syn_scanner_t *scanner, uint32_t target_ip,
...
int syn_scanner_send_batch(syn_scanner_t *scanner, size_t count) {
  ...
  for (size_t i = 0; i < count; i++) {
    ...
    uint16_t target_port =
        scanner->target_ports[get_random_uint32() % scanner->port_count];
 ...
 */
int syn_scanner_recv_synacks(syn_scanner_t *scanner, synack_callback_t callback,
                             void *user_data) {
  if (!scanner) {
    return -1;
  }
  ...
  for (int i = 0; i < nfds; i++) {
    ...
    if (parse_synack(scanner, buffer, n, &src_ip, &src_port)) {
      ...
      log_debug("SYN-ACK from %s:%u", inet_ntoa(*(struct in_addr *)&src_ip),
                src_port);
    }
  }
