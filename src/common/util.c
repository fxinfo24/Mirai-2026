...
void util_free(void *ptr) { free(ptr); }

size_t util_strlen(const char *str) { return str ? strlen(str) : 0; }
...
void util_random_bytes(void *buf, size_t len) { randombytes_buf(buf, len); }

uint32_t util_random_u32(void) { return randombytes_uniform(UINT32_MAX); }
