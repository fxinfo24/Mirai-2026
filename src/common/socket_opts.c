 ...
 */
int socket_set_keepalive(int sockfd, int idle_sec __attribute__((unused)),
...
#ifdef TCP_KEEPIDLE
  if (setsockopt(sockfd, IPPROTO_TCP, TCP_KEEPIDLE, &idle_sec,
                 sizeof(idle_sec)) < 0) {
...
#ifdef TCP_KEEPINTVL
  if (setsockopt(sockfd, IPPROTO_TCP, TCP_KEEPINTVL, &interval_sec,
...
#ifdef TCP_FASTOPEN
  if (setsockopt(sockfd, IPPROTO_TCP, TCP_FASTOPEN, &queue_len,
                 sizeof(queue_len)) < 0) {
...
int socket_set_timeout(int sockfd, int send_timeout_sec, int recv_timeout_sec) {
  ...
  if (setsockopt(sockfd, SOL_SOCKET, SO_SNDTIMEO, &send_tv, sizeof(send_tv)) <
      0) {
    log_error("SO_SNDTIMEO failed: %s", strerror(errno));
    return -1;
  }

  if (setsockopt(sockfd, SOL_SOCKET, SO_RCVTIMEO, &recv_tv, sizeof(recv_tv)) <
      0) {
