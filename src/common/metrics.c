...
metrics_context_t *metrics_init(uint16_t port) {
  ...
  ctx->running = true;
  if (pthread_create(&ctx->server_thread, NULL, metrics_http_server, ctx) !=
      0) {
...
static void metrics_handle_request(metrics_context_t *ctx, int client_fd) {
  ...
  for (size_t i = 0; i < ctx->metric_count; i++) {
    if (!ctx->metrics[i].active)
      continue;

    const char *type_str = "counter";
    if (ctx->metrics[i].type == METRIC_GAUGE)
      type_str = "gauge";
    else if (ctx->metrics[i].type == METRIC_HISTOGRAM)
      type_str = "histogram";
    ...
    if (strlen(ctx->metrics[i].labels) > 0) {
      offset += snprintf(response + offset, HTTP_BUFFER_SIZE - offset,
                         "%s{%s} %.2f\n", ctx->metrics[i].name,
                         ctx->metrics[i].labels, ctx->metrics[i].value);
    } else {
      offset +=
          snprintf(response + offset, HTTP_BUFFER_SIZE - offset, "%s %.2f\n",
                   ctx->metrics[i].name, ctx->metrics[i].value);
    }
  }
...
void metrics_counter_inc(metrics_context_t *ctx, const char *name,
  ...
  metric_entry_t *metric =
      metrics_find_or_create(ctx, name, labels, METRIC_COUNTER);
  if (metric) {
    metric->value += value;
  }
...
void metrics_gauge_set(metrics_context_t *ctx, const char *name,
  ...
  metric_entry_t *metric =
      metrics_find_or_create(ctx, name, labels, METRIC_GAUGE);
  if (metric) {
    metric->value = value;
  }
...
void metrics_histogram_observe(metrics_context_t *ctx, const char *name,
  ...
  metric_entry_t *sum_metric =
      metrics_find_or_create(ctx, sum_name, labels, METRIC_COUNTER);
  metric_entry_t *count_metric =
      metrics_find_or_create(ctx, count_name, labels, METRIC_COUNTER);
