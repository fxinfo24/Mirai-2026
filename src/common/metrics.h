...
#define METRIC_DETECTION(ctx, type)                                            \
  metrics_counter_inc(ctx, "mirai_bot_detections_total", "type=\"" type "\"",  \
                      1.0)

#define METRIC_ACTIVE_CONNECTIONS(ctx, count)                                  \
  metrics_gauge_set(ctx, "mirai_scanner_active_connections", NULL,             \
                    (double)(count))

#define METRIC_BANDWIDTH_UTIL(ctx, percent)                                    \
  metrics_gauge_set(ctx, "mirai_bot_bandwidth_utilization_percent", NULL,      \
                    (double)(percent))
