...
bool kill_switch_check(kill_switch_t *ks) {
    ...
    if (res != CURLE_OK) {
        ...
        if (ks->consecutive_failures >= 3) {
            log_error("Kill switch unreachable for 3 consecutive checks - terminating for safety");
            return true;
        }
        return false;
    }
...
bool time_limit_exceeded(time_limit_t *tl) {
    ...
    if (elapsed > tl->max_runtime_seconds) {
        log_warn("Time limit exceeded: %ld seconds (max: %ld)",
                 elapsed, tl->max_runtime_seconds);
        return true;
    }
