...
static const char *audit_event_name(audit_event_t event) {
  ...
  case AUDIT_SCAN_START:
    return "SCAN_START";
  case AUDIT_SCAN_STOP:
    return "SCAN_STOP";
  case AUDIT_DEVICE_FOUND:
    return "DEVICE_FOUND";
  case AUDIT_CREDENTIAL_ATTEMPT:
    return "CREDENTIAL_ATTEMPT";
  case AUDIT_DEVICE_COMPROMISED:
    return "DEVICE_COMPROMISED";
  case AUDIT_ATTACK_LAUNCHED:
    return "ATTACK_LAUNCHED";
  case AUDIT_KILL_SWITCH_ACTIVATED:
    return "KILL_SWITCH_ACTIVATED";
  case AUDIT_HONEYPOT_DEPLOYED:
    return "HONEYPOT_DEPLOYED";
  case AUDIT_HONEYPOT_STOPPED:
    return "HONEYPOT_STOPPED";
  case AUDIT_DATA_COLLECTED:
    return "DATA_COLLECTED";
  case AUDIT_NETWORK_VIOLATION:
    return "NETWORK_VIOLATION";
  case AUDIT_OPERATION_DENIED:
...
int audit_log_init(const char *log_path, const char *researcher,
                   const char *project) {
  if (!log_path) {
...
void audit_log_with_target(audit_event_t event, const char *target,
  ...
  json_object_object_add(jobj, "event",
                         json_object_new_string(audit_event_name(event)));

  if (strlen(researcher_id) > 0) {
    json_object_object_add(jobj, "researcher_id",
                           json_object_new_string(researcher_id));
  }
  if (strlen(project_id) > 0) {
    json_object_object_add(jobj, "project_id",
...
void audit_log_token(const char *token) {
  ...
  json_object_object_add(jobj, "timestamp", json_object_new_string(timestamp));
  json_object_object_add(jobj, "event",
                         json_object_new_string("AUTHORIZATION_TOKEN"));
