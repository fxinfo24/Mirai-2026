...
result_t crypto_encrypt(const uint8_t *plaintext, size_t plaintext_len, const uint8_t *key,
                       uint8_t **ciphertext, size_t *ciphertext_len)
{
    ...
    size_t total_len = crypto_aead_chacha20poly1305_IETF_NPUBBYTES + plaintext_len +
    ...
    *ciphertext_len = total_len;
    ...
    if (crypto_aead_chacha20poly1305_ietf_encrypt(
            *ciphertext + crypto_aead_chacha20poly1305_IETF_NPUBBYTES, &ciphertext_only_len,
            plaintext, plaintext_len, NULL, 0, NULL, nonce, key) != 0) {
...
result_t crypto_decrypt(const uint8_t *ciphertext, size_t ciphertext_len, const uint8_t *key,
                       uint8_t **plaintext, size_t *plaintext_len)
{
    ...
    const uint8_t *encrypted_data = ciphertext + crypto_aead_chacha20poly1305_IETF_NPUBBYTES;
    size_t encrypted_len = ciphertext_len - crypto_aead_chacha20poly1305_IETF_NPUBBYTES;
    ...
    *plaintext = util_malloc(*plaintext_len);
    ...
    if (crypto_aead_chacha20poly1305_ietf_decrypt(*plaintext, &plaintext_actual_len, NULL,
                                                  encrypted_data, encrypted_len, NULL, 0, nonce,
                                                  key) != 0) {
        util_free(*plaintext);
