...
auth_config_t *auth_init(const char *config_file) {
  ...
  if (json_object_object_get_ex(root, "authorized_operations", &ops_obj)) {
    ...
    auth->num_operations =
        ops_len < MAX_AUTHORIZED_OPS ? ops_len : MAX_AUTHORIZED_OPS;
  ...
  if (json_object_object_get_ex(root, "network_restrictions", &networks_obj)) {
    ...
    auth->num_networks =
        net_len < MAX_NETWORK_RANGES ? net_len : MAX_NETWORK_RANGES;
...
bool auth_verify(auth_config_t *auth) {
  ...
  log_info("Authorization verified: %s/%s", auth->researcher_id,
           auth->project_id);
  return true;
}
...
bool auth_check_operation_type(auth_config_t *auth, operation_type_t op_type) {
  const char *op_names[] = {"scan:local_network", "scan:internet",
                            "honeypot:deploy",    "honeypot:monitor",
                            "analysis:passive",   "analysis:active",
                            "attack:test",        "credential:test",
...
bool auth_check_network(auth_config_t *auth, const char *target_ip) {
  ...
  for (int i = 0; i < auth->num_networks; i++) {
    if (ip_in_cidr(target_ip, auth->allowed_networks[i])) {
      log_debug("Network authorized: %s in %s", target_ip,
                auth->allowed_networks[i]);
      return true;
    }
  }
...
bool auth_runtime_exceeded(auth_config_t *auth, time_t start_time) {
  ...
  if (elapsed_hours > auth->max_runtime_hours) {
    log_error("Runtime limit exceeded: %ld hours > %ld hours", elapsed_hours,
              auth->max_runtime_hours);
    return true;
  }
...
#pragma GCC diagnostic ignored "-Wformat-truncation"
  snprintf(status, sizeof(status),
           "Researcher: %s | Project: %s | Expires in: %ld hours | Operations: "
           "%d | Networks: %d",
           auth->researcher_id, auth->project_id, remaining / 3600,
           auth->num_operations, auth->num_networks);
